...
def emit_registry():
    ...
    as a script the pages load."""
    sys.path.insert(0, str(ROOT / "tools"))
    import manifests
    items, problems = manifests.load_all()
    for msg in problems:
        print(f"manifest: {msg}")
    if problems:
        sys.exit("invalid manifests — see canon/**/manifest.yml")
    data = manifests.index(items)
    w(ROOT / "canon" / "registry.json", json.dumps(data, ensure_ascii=False, indent=1) + "\n")
    w(DOCS / "registry.data.js", "/* yoshiki registry — generated by tools/build.py. Do not edit. */\n"
      "window.YOSHIKI_REGISTRY = " + json.dumps(data, ensure_ascii=False) + ";\n")

    # A shadcn-shaped registry beside it: any item that declares files can be installed with
    # `npx shadcn add`, which is how agents and people pull a piece into their own project.
    shad = {"$schema": "https://ui.shadcn.com/schema/registry.json",
            "name": "yoshiki", "homepage": "https://zinzaki.github.io/yoshiki/",
            "items": [{"name": it["id"].split("/")[-1].replace(".", "-"),
                       "type": (it["files"][0].get("type") or "registry:ui"),
                       "title": it.get("title", ""),
                       "description": it.get("summary", ""),
                       "files": it["files"],
                       **({"registryDependencies": it["registryDependencies"]} if it.get("registryDependencies") else {}),
                       **({"dependencies": it["dependencies"]} if it.get("dependencies") else {})}
                      for it in items if it.get("files")]}
    w(ROOT / "registry.json", json.dumps(shad, ensure_ascii=False, indent=2) + "\n")
